document.addEventListener('DOMContentLoaded', function () {
  ...
  const saveButton = document.getElementById('save-item'); // Corrected from 'save-items' to 'save-item'
  ...
  saveButton.addEventListener('click', function () {
    ...
    // Clear the item preview after saving
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      chrome.storage.sync.get('savedItems', function (data) {
        ...
        chrome.storage.sync.set({ savedItems }, function () {
          // Update the shopping list after saving
          updateShoppingList();
  ...
  function deleteItem(urlToDelete) {
    chrome.storage.sync.get('savedItems', function (data) {
      ...
      chrome.storage.sync.set({ savedItems: updatedItems }, function () {
        // Update the shopping list after deletion
        updateShoppingList();
      });
    });
  }

  // Initial call to load saved items when the popup is opened
  updateShoppingList();
});
